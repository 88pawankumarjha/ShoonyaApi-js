...
const maxLossThreshold = -1.5;
const maxProfitThreshold = 2;
...
async function XEma(fastEMA, slowEMA) {
  ...
  if (!positionTaken) {
    ...
    if (diff > 5) {
      send_notification(`SELL PUT: fastEMA (${fastEMA}) - slowEMA (${slowEMA}) = ${diff} > 5`);
      await short(biasProcess.atmPutSymbol, globalInput.LotSize * globalInput.emaLotMultiplier);
      positionTakenInSymbol = biasProcess.atmPutSymbol;
      positionTaken = true;
    ...
    if (diff < -5) {
      send_notification(`SELL CALL: fastEMA (${fastEMA}) - slowEMA (${slowEMA}) = ${diff} < -5`);
      await short(biasProcess.atmCallSymbol, globalInput.LotSize * globalInput.emaLotMultiplier);
      positionTakenInSymbol = biasProcess.atmCallSymbol;
      positionTaken = true;
    ...
    // Buy PUT when diff <= 5
    if (positionDirection == 'long' && (diff <= 5)) {
      send_notification(`BUY PUT: fastEMA (${fastEMA}) - slowEMA (${slowEMA}) = ${diff} <= 5`);
      await long(positionTakenInSymbol, globalInput.LotSize * globalInput.emaLotMultiplier);
      positionTaken = false;
    }
    // Buy CALL when diff >= -5
    else if (positionDirection == 'short' && (diff >= -5)) {
      send_notification(`BUY CALL: fastEMA (${fastEMA}) - slowEMA (${slowEMA}) = ${diff} >= -5`);
      await long(positionTakenInSymbol, globalInput.LotSize * globalInput.emaLotMultiplier);
      positionTaken = false;
    }
  }
...
runEma = async () => {
  try {
    ...
    globalInput.emaLotMultiplier = limits?.cash < 800000 ? 1 : 2;
